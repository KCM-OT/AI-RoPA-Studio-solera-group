...
export const SEED_ACTIVITIES: ProcessingActivity[] = [
  {
    ...
    relationships: [
      ...
      {
        id: 'sg-recruit-checkr-repo',
        type: 'asset',
        name: 'Background Check Repository',
        inventoryId: 'ast-background',
        provenance: 'ai',
        confidence: 0.83,
        status: 'suggested',
        evidence: 'Checkr results are stored in the background check repository for two years',
        rationale:
          'The DPIA describes Checkr writing screening results to a dedicated store, but no asset is linked for it yet.',
        sourceDocId: 'sd-recruit-dpia',
      },
      {
        id: 'sg-recruit-hirevue',
        type: 'vendor',
        name: 'HireVue',
        inventoryId: null,
        provenance: 'ai',
        confidence: 0.64,
        status: 'suggested',
        evidence: 'recorded interviews are scored by HireVue before Talent Acquisition review',
        rationale:
          'A processor named in the rollout brief that is not in your vendor inventory. Confirm before linking.',
        sourceDocId: 'sd-recruit-brief',
      },
      {
        id: 'sg-recruit-biometric',
        type: 'personalData',
        name: 'Biometric data',
        inventoryId: 'pdc-biometric',
        provenance: 'ai',
        confidence: 0.52,
        status: 'suggested',
        evidence: 'facial and vocal analysis of recorded interviews',
        rationale:
          'Interview scoring may derive biometric identifiers — a special category needing explicit review.',
        sourceDocId: 'sd-recruit-dpia',
      },
    ],
    sourceDocuments: [
  ...
  },
  {
    ...
    relationships: [
      ...
      pdcRel('pdc-email', 'Email address'),
      {
        id: 'sg-marketing-salesforce',
        type: 'vendor',
        name: 'Salesforce',
        inventoryId: 'ven-salesforce',
        provenance: 'ai',
        confidence: 0.67,
        status: 'suggested',
        evidence: 'campaign engagement is synced back to Salesforce',
        rationale:
          'Engagement analytics appear to feed an existing CRM vendor. Confirm the processor relationship.',
        sourceDocId: 'sd1',
      },
    ],
  ...
  },
  {
    ...
    relationships: [
      ...
      pdcRel('pdc-device', 'Device identifiers'),
      {
        ...
        confidence: 0.9,
        status: 'suggested',
        evidence: 'website usage is collected through Google Analytics tags',
        rationale:
          'A high-confidence processor detected for this activity, which currently has no vendor linked. Not yet in your inventory.',
      },
      {
        id: 'sg-analytics-warehouse',
        type: 'asset',
        name: 'People Analytics Warehouse',
        inventoryId: 'ast-warehouse',
        provenance: 'ai',
        confidence: 0.71,
        status: 'suggested',
        evidence: 'aggregated traffic metrics land in the analytics warehouse',
        rationale:
          'Traffic data appears to flow into an existing warehouse asset. Linking it closes this record’s asset gap.',
      },
      {
        id: 'sg-analytics-location',
        type: 'personalData',
        name: 'Location data',
        inventoryId: 'pdc-location',
        provenance: 'ai',
        confidence: 0.61,
        status: 'suggested',
        evidence: 'approximate visitor geography derived from IP',
        rationale:
          'IP-derived geography may qualify as location data. Confirm whether it is retained.',
      },
    ],
    sourceDocuments: [],
    createdWithAI: false,
  ...
  },
  {
    ...
    relationships: [
      ...
      pdcRel('pdc-address', 'Postal address'),
      {
        ...
        confidence: 0.74,
        status: 'suggested',
        evidence: 'the payment gateway runs on AWS in us-east-1',
        rationale:
          'A sub-processor providing hosting for this activity. Linking it keeps the transfer picture complete.',
      },
    ],
    sourceDocuments: [],
    createdWithAI: false,
